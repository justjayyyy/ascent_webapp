    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={cn(colors.cardBg, colors.cardBorder, "max-w-2xl max-h-[90vh] overflow-y-auto")}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className={colors.textSecondary}>{t('categoryName') || 'Category Name'} *</Label>
              <Input
                type="text"
            ...
            <div className="space-y-2">
              <Label className={colors.textSecondary}>{t('type')} *</Label>
              <Select 
                value={formData.type} 
        ...
        <div className="mt-6 space-y-3">
          <h3 className={cn("font-semibold", colors.textPrimary)}>{t('allCategories') || 'All Categories'}</h3>
          
          {categories.length === 0 ? (
            <p className={cn("text-sm text-center py-4", colors.textTertiary)}>
                  ...
                  <CardContent className="p-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className={cn("text-xs px-2 py-0.5 rounded", colors.textTertiary, colors.bgTertiary)}>
